...
contract FundMeTest is Test {
    ...
    function testFundfailswithOutEnouhghEth() public {
        vm.expectRevert(); // the next line of code should revert, if it doesn't the test will fail
        fundMe.fund(); // we are not sending any eth, so it should revert
    }

    function testFundUpdatesDataSturacture() public {
        vm.prank(USER); // the next txn will be sent by USER
        fundMe.fund{value: SEND_VALUE}(); // we are sending 10 eth, so it should update the data structure
